...
for i in range(10): 
    ...
    ca = ca0[(i*len(y0)/cell):(i*len(y0)/cell+len(y0)/cell)-1,:]
    ca = np.append(ca, ca[-1:,:], axis=0)
    ca = np.append(ca, ca[:,-1:], axis=1)
    ...
    contours = np.arange(np.min(ca0),np.max(ca0)+(np.max(ca0)-np.min(ca0))/10.0,
                         (np.max(ca0)-np.min(ca0))/10.0)
